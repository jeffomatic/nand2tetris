...
commands.each do |c, line|
  ...
  when "label"   then label_command(args, line)
  when "goto"    then goto_command(args, line)
  when "if-goto" then if_goto_command(args, line)
...
def comparison_command(args : Array(String), line : Int, command : String, jump : String)
  ...
  puts "@__cond_equal__#{line}"  # load jump label address
  puts "D;#{jump}"               # jump on comparison
  puts "D=0"                     # not equal
  puts "@__cond_done__#{line}"   # load done
  puts "0;JMP"                   # goto done
  puts "(__cond_equal__#{line})" # equal label label
  puts "D=-1"                    # equal
  puts "(__cond_done__#{line})"  # done label
  decrement_stack_pointer
  puts "A=M-1" # move backward (address of first operand)
  puts "M=D"   # overwrite first operand with equality value
end

def label_command(args : Array(String), line : Int)
  check_arg_count(args, line, 1, "label")
  puts "(__label__#{args[0]})"
end

def goto_command(args : Array(String), line : Int)
  check_arg_count(args, line, 1, "goto")
  puts "@__label__#{args[0]}"
  puts "0;JMP"
end

def if_goto_command(args : Array(String), line : Int)
  check_arg_count(args, line, 1, "if-goto")
